...
def get_user_input_and_run():
    ...
    # Parse the hidden layer neurons as a list
    hidden_layer_neurons_str = neurons_entry.get()
    hidden_layer_neurons = list(map(int, hidden_layer_neurons_str.split(',')))
    ...
    conf_matrix, accuracy = main.run_neural_network(epochs, num_hidden_layers, hidden_layer_neurons,
                                                    activation_function, learning_rate, bias)
...
neurons_label = ttk.Label(root, text="Number of Neurons in Each Hidden Layer (comma-separated) Like this h1,h2,...hi:")
...
activation_combobox = ttk.Combobox(root, textvariable=activation_var, values=["Sigmoid", "hyperbolic tangent"])
activation_combobox.grid(row=5, column=1, padx=10, pady=5)

run_button = ttk.Button(root, text="(Run Back Propagation) ", command=get_user_input_and_run)
